...
// WORDBITS is the bits of a word
...
func (s *IntSet) Copy() *IntSet {
	snew := &IntSet{words: make([]uint, len(s.words))}
...
func (s *IntSet) Elems() []int {
	...
	for i, word := range s.words {
		...
		for j := uint(0); j < WORDBITS; j++ {
			if word&(1<<j) != 0 {
				elems = append(elems, i*WORDBITS+int(j))
			}
		}
	}
	return elems
